...
def read_key(key_path: str) -> bytes:
    try:
        ...
        try:
            ...
            if lines[0] == KEY_HEADER and lines[-1] == KEY_FOOTER:
                ...
                if len(key_bytes) == EXPECTED_KEY_LEN:
                    ...
                    raise ValueError(
                        f"Неправильна довжина ключа в ASCII файлі: {len(key_bytes)}, очікувалось {EXPECTED_KEY_LEN}"
                    )
        ...
        if len(content_bytes) == EXPECTED_KEY_LEN:
            ...
            raise ValueError(
                f"Неправильна довжина ключа у файлі: {len(content_bytes)}, очікувалось {EXPECTED_KEY_LEN} байт (або ASCII формат)"
            )
...
def read_input(input_path: str | None) -> bytes:
    try:
        ...
        print(
            f"Помилка під час читання вхідних даних '{input_path}': {e}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def write_output(
    data: bytes, output_path: str | None, ascii_format: bool, data_type: str
):
    ...
    if ascii_format:
        ...
        if output_path is None or output_path == "-":
            ...
            try:
                ...
                print(
                    f"Помилка під час запису у файл '{output_path}': {e}",
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        if output_path is None or output_path == "-":
            try:
                ...
                print(
                    f"Помилка під час запису в stdout (бінарний режим): {e}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
...
def handle_generate_key(args: argparse.Namespace):
    key_bytes = np.random.randint(
        0, 256, size=(EXPECTED_KEY_LEN,), dtype=np.uint8
    ).tobytes()
...
def main():
    parser = argparse.ArgumentParser(
        description="NAL Encryption CLI інструмент.", prog="nalenc"
    )
    parser.add_argument(
        "-v", "--version", action="version", version=f"%(prog)s {nalenc_version}"
    )
    ...
    parser_genkey = subparsers.add_parser(
        "generate-key", help="Згенерувати новий ключ шифрування (512 байт)"
    )
    parser_genkey.add_argument(
        ...
        type=str,
        default=None,
        help="Файл для виведення ключа (за замовчуванням: stdout)",
    )
    parser_genkey.add_argument(
        "-a",
        "--ascii",
        action="store_true",
        help="Вивести ключ у форматі ASCII (Base64 із заголовками)",
    )
    ...
    parser_encrypt.add_argument(
        "-k", "--key", type=str, required=True, help="Файл з ключем шифрування"
    )
    parser_encrypt.add_argument(
        "-o",
        "--output",
        type=str,
        default=None,
        help="Файл для виведення результату (за замовчуванням: stdout)",
    )
    parser_encrypt.add_argument(
        "-a",
        "--ascii",
        action="store_true",
        help="Вивести результат у форматі ASCII (Base64 із заголовками)",
    )
    parser_encrypt.add_argument(
        "input_file",
        type=str,
        nargs="?",
        default=None,
        help="Вхідний файл для шифрування (за замовчуванням: stdin)",
    )
    ...
    parser_decrypt.add_argument(
        ...
        "--ascii",
        action="store_true",
        help="Вивести результат у форматі ASCII (Base64 із заголовками). Вхідні дані також можуть бути в цьому форматі.",
    )
    parser_decrypt.add_argument(
        "input_file",
        type=str,
        nargs="?",
        default=None,
        help="Вхідний файл для розшифрування (за замовчуванням: stdin)",
    )
