...
class NALEnc:
    def __init__(self, passwd: input_type):
        ...
        if len(passwd_encoded) != 512:
            raise ValueError("passwd len must equal 512 byte")
    ...
    def encrypt(self, msg: input_type) -> list[int]:
        ...
        parts = (
            encrypt_loop_uint8(parts, self.__prepared_passwds)
            if len(parts[0]) < 65536
            else encrypt_loop_uint64(parts, self.__prepared_passwds)
        )
    ...
    def decrypt(self, msg: input_type) -> list[int]:
        ...
        parts = (
            decrypt_loop_uint8(parts, self.__prepared_passwds)
            if len(parts[0]) < 65536
            else decrypt_loop_uint64(parts, self.__prepared_passwds)
        )
    ...
    def __prepare_passwds(self) -> None:
        ...
        self.__prepared_passwds[1] = np.where(
            idx_array != 0,
        ...
        for i in range(1, 255):
            ...
            self.__prepared_passwds[i + 1] = np.where(
                idx_array != i,
                self.__prepared_passwds[i - 1] ^ xor_value,
    ...
    def __encode_value(value: input_type) -> npt.NDArray[np.uint8]:
        try:
            ...
            raise TypeError(
                "Argument must be str | bytes | Iterable[int] | NDArray[uint8]"
            )
